...
const liquidbuttonVariants = cva(
  "relative group inline-flex items-center transition-all justify-center cursor-pointer gap-2 whitespace-nowrap rounded-full text-sm font-medium transition-[color,box-shadow,transform] disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/50",
  {
    variants: {
      variant: {
        default: "bg-transparent text-white hover:scale-[1.03] active:scale-[0.98]",
        destructive:
  ...
  return (
    <>
      <Comp
        ...
        {...props}
      >
        {/* Glass Background & Refraction Layer (Dynamic Tint Support) */}
        <div className={cn(
          "absolute inset-0 z-0 rounded-full backdrop-blur-2xl border transition-all duration-300",
          glassClassName || "bg-white/10 border-white/40 shadow-[0_4px_15px_rgba(255,255,255,0.2),inset_0_1px_1.5px_rgba(255,255,255,0.7)] group-hover:bg-white/20 group-hover:border-white/60"
        )} />
        
        {/* Top Gloss Sheen & High-Shine Reflection */}
        <div className="absolute inset-x-1 top-0.5 h-1/2 rounded-t-full bg-gradient-to-b from-white/50 via-white/15 to-transparent pointer-events-none z-0" />
        <div className="absolute inset-0 rounded-full ring-1 ring-inset ring-white/50 pointer-events-none z-0" />
