...
import { Database, Tag, Clock, Search, RefreshCcw, Network, List } from "lucide-react";
      ...
      <section className="bg-linear-to-br from-zinc-900 to-black border border-zinc-800 p-6 md:p-8 rounded-2xl">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between mb-6">
          ...
          <div className="flex flex-wrap gap-2 w-full sm:w-auto items-center">
            {/* View Mode Toggle */}
            <div className="flex bg-zinc-900 border border-zinc-700 p-1 rounded-xl">
              <button
                onClick={() => setViewMode('graph')}
                className={`px-3 py-1.5 rounded-lg text-[10px] md:text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 transition-colors cursor-pointer ${
                  viewMode === 'graph'
                    ? 'bg-zinc-800 text-isro-orange border border-zinc-700'
            ...
            <button
              ...
              className="px-4 py-3 rounded-xl bg-isro-orange hover:bg-orange-500 text-white text-xs font-bold uppercase tracking-wider inline-flex items-center gap-2 cursor-pointer"
        ...
        {viewMode === 'graph' ? (
          liveNodes.length > 0 ? (
            <GraphVisualizer nodes={liveNodes} onQueryNode={onQueryChunk} />
          ) : (
            !isLoadingNodes && (
              <div className="p-12 rounded-xl border border-zinc-800 bg-zinc-900/40 text-zinc-500 text-sm text-center">
                No live nodes indexed for "{searchQuery}". Try searching for another keyword.
              </div>
            )
          )
        ) : (
